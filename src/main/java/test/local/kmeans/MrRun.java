...
/**
 * XXX: 除了 TEST_DATE_YMD 取值以外请勿修改其他代码！M/R 任务相关配置请修改 main/resources/mbp_mr.yaml 配置文件
 * 
...
public class MrRun implements Tool
{
	...
	public static final String TEST_DATE_YMD = "20130713";
	public static final String MBP_MR = "test/local/kmeans/mbp_mr.yaml";
	...
	private static SimpleDateFormat dayFormat1 = new SimpleDateFormat("yyyy-MM-dd");
	private static SimpleDateFormat dayFormat2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	{
		Pattern dateYmdPat = Pattern.compile("\\$\\{\\s*date_ymd\\s*(([+-])\\s*(\\d+))?\\s*\\}");
		String res = str;
	...
	public static Map loadMrConf(String confName) throws Exception
	{
		...
		if (null == url)
			...
			LOG.info(url);
		InputStream is = MrRun.class.getClassLoader().getResourceAsStream(confName);
	...
	public Job makeMrJob(Map mr_conf, Date dateYmd) throws Exception
	{
		...
		job.setJobName(mr_conf.get("prog_name") + "_" + mr_conf.get("app_key") + "_" + sdf.format(dateYmd) + "_" + dayFormat2.format(System.currentTimeMillis()));
		job.setJarByClass(getClass());

		// mr_classes
		Map classes = (Map) mr_conf.get("mr_classes");

		if (!classes.containsKey("mapper") || !classes.containsKey("mapper_output_key") || !classes.containsKey("mapper_output_value"))
		{
		...
		Class<?> mapperOutputKeyClazz = Class.forName((String) classes.get("mapper_output_key"));
		Class<?> mapperOutputValClazz = Class.forName((String) classes.get("mapper_output_value"));

		if (mr_conf.containsKey("min_split_size"))
		{
			int min_split_size = (Integer) mr_conf.get("min_split_size");
			job.getConfiguration().setLong(CombineFileInputFormat.SPLIT_MINSIZE_PERNODE, min_split_size * 1024 * 1024);
			job.getConfiguration().setLong(CombineFileInputFormat.SPLIT_MINSIZE_PERRACK, min_split_size * 1024 * 1024);
		}
		if (mr_conf.containsKey("min_split_size"))
		{
			int max_split_size = (Integer) mr_conf.get("max_split_size");
			job.getConfiguration().setLong("mapreduce.input.fileinputformat.split.maxsize", max_split_size * 1024 * 1024);
		}
		...
		{
			Class<?> reducerClazz = Class.forName((String) classes.get("reducer"));
		...
		{
			Class<?> combinerClazz = Class.forName((String) classes.get("combiner"));
			job.setCombinerClass((Class<? extends Reducer>) combinerClazz);
		}
		// 增加可选的 partitioner
		if (classes.containsKey("partitioner"))
		{
			Class<?> partitionerClazz = Class.forName((String) classes.get("partitioner"));
		...
		{
			Class<?> keyComparatorClazz = Class.forName((String) classes.get("key_comparator"));
		...
		{
			Class<?> valGroupComparatorClazz = Class.forName((String) classes.get("value_grouping_comparator"));
		...
		for (Object o : inTabs.entrySet())
		{
			...
			job.getConfiguration().set("mapred.input.table", tabNames == null ? tabNameStr : tabNames + "," + tabNameStr);

			if (entry.getValue() == null)
			{
				// 无指定输入表的分区
				TableFlatInputFormat.addInputPath(job, new Path((TABLE_PARENT + inTabName)));
			}
			else if (entry.getValue() instanceof String)
			{
				...
				TableFlatInputFormat.addInputPath(job, new Path((TABLE_PARENT + inTabName + "/" + expandedPartStr)));
			}
			else
			{
				// 指定了输入表的多个分区列表
		...
		if (mr_conf.containsKey("db_out"))
		{
			...
			DBConfiguration.configureDB(job.getConfiguration(), driver_class, db_url, user_name, passwd);
			...
			if (!mr_conf.containsKey("table_out"))
			{
				...
				try
				{
					...
					statement = connection.prepareStatement("DELETE FROM " + db_table);
				...
				catch (Exception e)
				{
					...
					{
						LOG.warn(org.apache.hadoop.util.StringUtils.stringifyException(ex));
					}
		...
		if (mr_conf.containsKey("table_out"))
		{
			...
			else
			{
				...
				for (Object o : outTab.entrySet())
				{
					...
					String partStr = (String) entry.getValue();
					String expandedPartStr = expandMacroDateYmd(partStr, dateYmd);
					Path outPath = new Path((TABLE_PARENT + outTabName + "/" + expandedPartStr));
					FileSystem fsdl = outPath.getFileSystem(getConf());
					fsdl.delete(outPath, true);
					TableOutputFormat.setOutputPath(job, outPath);
	 ...
	 * @throws Exception
	 */
	public ControlledJob makeControlledJob(Map mr_conf, String[] args) throws Exception
	{
		...
		jobConf.set("fs.default.name", "file:///home/party/workspace/mooncloud/warehouse/");
