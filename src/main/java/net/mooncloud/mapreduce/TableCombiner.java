 ...
 */
public class TableCombiner<KEYOUT, VALUEOUT> extends Reducer<KEYOUT, VALUEOUT, KEYOUT, VALUEOUT> {
	 ...
	 */
	protected void reduce(KEYOUT key, Iterable<VALUEOUT> values, Context context) throws IOException,
			InterruptedException {
		// NOTHING
		combine(key, values, context);
	}
	
	protected void combine(KEYOUT key, Iterable<VALUEOUT> values, Context context) throws IOException,
			InterruptedException {
		// NOTHING
	}
	...
	public void run(Context context) throws IOException, InterruptedException {
		...
		try {
			while (context.nextKey()) {
				...
				Iterator<VALUEOUT> iter = context.getValues().iterator();
				if (iter instanceof ReduceContext.ValueIterator) {
					((ReduceContext.ValueIterator<KEYOUT>) iter).resetBackupStore();
				}
			}
		} finally {
			cleanup(context);
