...
public class TableCombineInputFormat extends CombineFileInputFormat<InputSplitFile, Record> {
	@Override
	public RecordReader<InputSplitFile, Record> createRecordReader(InputSplit split, TaskAttemptContext context)
			throws IOException {
		return new CombineFileRecordReader<InputSplitFile, Record>((CombineFileSplit) split, context,
...
class TableRecordReader extends RecordReader<InputSplitFile, Record> {
	...
	public TableRecordReader(CombineFileSplit split, TaskAttemptContext context, Integer index) throws IOException {
	...
	private void initializeTabelSchema(Configuration job, final Path file) throws IOException {
		FileSystem fs = file.getFileSystem(job);
		...
		// table
		String[] tabNames = job.getStrings("mapred.input.table", new String[] {});
		...
		if (null == tablePath || !tablePath.equals(tablePathT)) {
			...
			// tablePathT = ymdPathT;
			// schemaPath = new Path(ymdPathT, "__schema__");
			// }
			if (!fs.isFile(schemaPath)) {
				// schema = job.get("schema"); // schema = "record:string";
				// if (schema == null)
				throw new IOException("table: " + tablePathT.getName() + " not find file __schema__");
			} else {
			...
			if (schema.endsWith(":")) {
				...
				table = table.replace('.', '/'); // .replace(System.getProperty("odps.project.name", "mr_dw"), "")
				// if (table.contains("."))
				// {
				// table = table.substring(table.lastIndexOf('.'));
				// }
				Path t = new Path(job.get("table_parent", "") + table);
				if (!(t.equals(tablePath) || tablePath.toString().endsWith(t.toString()) || t.toString().endsWith(
						tablePath.toString()))) {
					throw new IOException("table: " + tablePathT + " not equals " + t + " of __schema__");
				}
	...
	public boolean nextKeyValue() throws IOException {
		if (key == null
				|| !(splitPath.getName().equals(key.getFileName()) && tablePath.getName().equals(key.getTableName()))) {
		...
		while (pos < end) {
			...
			String[] values = valueText.toString().split(defaultValueDelimiter); // , value.getFields().length);
			try {
				value.setAll(values);
			} catch (Exception e) {
				value = null;
