...
public class MrRun implements Tool {
	...
	public static String TEST_DATE_YMD = "20150624";
	public static String MBP_MR = "cdrcall/statistics/p0/mbp_mr.yaml";
...
//		}
		...
		if (classes.containsKey("reducer")) {
			...
			job.setNumReduceTasks(job.getConfiguration().getInt("reduce_tasks",
					(Integer) mr_conf.get("reduce_tasks")));
		} else {
			job.setNumReduceTasks(0);
			job.setOutputKeyClass(NullWritable.class);
		}
		LOG.info("mapred.reduce.tasks" + "=" + job.getNumReduceTasks());
		// 增加可选的 combiner
		...
		// 增加可选的 default_schema
		if (mr_conf.containsKey("default_schema")) {
			String default_schema = (String) mr_conf.get("default_schema");
			job.getConfiguration().set("default_schema", default_schema);
		}
		...
		LOG.info("mapred.input.dir="+job.getConfiguration().get("mapred.input.dir"));

		// setup_in
		inTabs = (Map) mr_conf.get("setup_in");
		if (inTabs != null && inTabs.size() > 0) {
			for (Object o : inTabs.entrySet()) {
				Entry entry = (Entry) o;
				String inTabName = ((String) entry.getKey()).replace('.', '/');
				...
				if (entry.getValue() == null) {
					// 无指定输入表的分区
					String tabNameStr = StringUtils.escapeString(inTabName);
					String tabNames = job.getConfiguration().get(
							"mapred.setup.table");
					job.getConfiguration().set(
							"mapred.setup.table",
							tabNames == null ? TABLE_PARENT + tabNameStr
									: tabNames + "," + TABLE_PARENT
											+ tabNameStr);
				} else if (entry.getValue() instanceof String) {
					// 指定了输入表的单个分区
					String partStr = (String) entry.getValue();
					...
					job.getConfiguration().set(
							"mapred.setup.table",
							tabNames == null ? TABLE_PARENT + tabNameStr + "/"
									+ expandedPartStr : tabNames + ","
									+ TABLE_PARENT + tabNameStr + "/"
									+ expandedPartStr);
				} else {
					// 指定了输入表的多个分区列表
					LOG.error("只允许指定初始表的一个分区！");
					return null;
				}
			}
		}
		...
		if (mr_conf.containsKey("table_out")) {
			...
			LOG.info("mapred.output.dir="+job.getConfiguration().get("mapred.output.dir"));
		}
	...
	public ControlledJob makeControlledJob(Map mr_conf, String[] args)
		...
		// 设置 MR 本地运行环境
		if (!jobConf.getBoolean("mapred.job.online", false)) {
